import type { NextPage } from "next";

const Testimonials: NextPage = () => {
  return (
    <section
      // className="box-border flex max-w-full flex-col items-start justify-start gap-[1.5rem] self-stretch overflow-hidden py-[3rem] pl-[1rem] pr-[0rem] text-left font-headings-h4-desktop text-[1.25rem] text-secondary-3 mq750:box-border mq750:pb-[1.94rem] mq750:pt-[1.94rem]"
      className="mq900:gap-[2rem] mq900:py-[3.25rem] mq900:px-[2.5rem] mq900:box-border box-border flex max-w-full flex-col items-center justify-start gap-[3rem] self-stretch overflow-hidden p-[5rem] text-left font-headings-h4-desktop text-[1.5rem] text-secondary-3"
      aria-labelledby="testimonials-heading"
    >
      <h2
        id="testimonials-heading"
        className="self-stretch text-[1.5rem] font-semibold leading-[1.75rem] text-secondary-3"
      >
        What Our Clients Say
      </h2>
      <div
        className="box-border flex h-[24.75rem] w-auto max-w-full shrink-0 flex-row items-start justify-start gap-[1.5rem] overflow-x-auto rounded-2xl bg-secondary-1 p-[1.5rem] text-[1rem] text-primary-1 mq450:box-border mq450:pb-[1.25rem] mq450:pt-[1.25rem]"
        role="list"
      >
        <article
          className="box-border flex min-h-[21.75rem] w-[20rem] shrink-0 flex-col items-start justify-between self-stretch overflow-hidden rounded-lg bg-primary-2 px-[1rem] py-[1.5rem]"
          role="listitem"
        >
          <div className="flex h-[11.75rem] flex-col items-start justify-start gap-[0.75rem] self-stretch">
            <h3 className="relative inline-block h-[2.5rem] self-stretch font-medium leading-[1.25rem]">
              &quot;The best people I have ever worked with&quot;
            </h3>
            <div className="relative flex-1 self-stretch font-paragraph-regular-semibold text-[0.88rem] leading-[1.25rem]">
              <p className="[margin-block-end:16px] [margin-block-start:0]">
                &quot;Incredible group of people and talented professionals.
                Focused on the development of flexible and new stuffs.
              </p>
              <p className="m-0">
                Very inspiring working experience with their representatives,
                responsible and active in communication&quot;
              </p>
            </div>
          </div>
          <footer className="font-satoshi flex h-[2.5rem] flex-col items-start justify-start gap-[0.5rem] self-stretch text-[1.25rem]">
            <div className="relative flex-1 self-stretch font-medium leading-[1rem]">
              Aniruddho Sen
            </div>
            <div className="relative flex-1 self-stretch text-[0.88rem] leading-[1rem]">
              Vice-president, SenIT
            </div>
          </footer>
        </article>
        <article
          className="box-border flex min-h-[21.75rem] w-[20rem] shrink-0 flex-col items-start justify-between self-stretch overflow-hidden rounded-lg bg-primary-2 px-[1rem] py-[1.5rem]"
          role="listitem"
        >
          <div className="flex h-[11.75rem] flex-col items-start justify-start gap-[0.75rem] self-stretch">
            <h3 className="relative inline-block h-[2.5rem] self-stretch font-medium leading-[1.25rem]">
              &quot;The best people I have ever worked with&quot;
            </h3>
            <div className="relative flex-1 self-stretch font-paragraph-regular-semibold text-[0.88rem] leading-[1.25rem]">
              <p className="[margin-block-end:16px] [margin-block-start:0]">
                &quot;Incredible group of people and talented professionals.
                Focused on the development of flexible and new stuffs.
              </p>
              <p className="m-0">
                Very inspiring working experience with their representatives,
                responsible and active in communication&quot;
              </p>
            </div>
          </div>
          <footer className="font-satoshi flex h-[2.5rem] flex-col items-start justify-start gap-[0.5rem] self-stretch text-[1.25rem]">
            <div className="relative flex-1 self-stretch font-medium leading-[1rem]">
              Aniruddho Sen
            </div>
            <div className="relative flex-1 self-stretch text-[0.88rem] leading-[1rem]">
              Vice-president, SenIT
            </div>
          </footer>
        </article>
        <article
          className="box-border flex min-h-[21.75rem] w-[20rem] shrink-0 flex-col items-start justify-between self-stretch overflow-hidden rounded-lg bg-primary-2 px-[1rem] py-[1.5rem]"
          role="listitem"
        >
          <div className="flex h-[11.75rem] flex-col items-start justify-start gap-[0.75rem] self-stretch">
            <h3 className="relative inline-block h-[2.5rem] self-stretch font-medium leading-[1.25rem]">
              &quot;The best people I have ever worked with&quot;
            </h3>
            <div className="relative flex-1 self-stretch font-paragraph-regular-semibold text-[0.88rem] leading-[1.25rem]">
              <p className="[margin-block-end:16px] [margin-block-start:0]">
                &quot;Incredible group of people and talented professionals.
                Focused on the development of flexible and new stuffs.
              </p>
              <p className="m-0">
                Very inspiring working experience with their representatives,
                responsible and active in communication&quot;
              </p>
            </div>
          </div>
          <footer className="font-satoshi flex h-[2.5rem] flex-col items-start justify-start gap-[0.5rem] self-stretch text-[1.25rem]">
            <div className="relative flex-1 self-stretch font-medium leading-[1rem]">
              Aniruddho Sen
            </div>
            <div className="relative flex-1 self-stretch text-[0.88rem] leading-[1rem]">
              Vice-president, SenIT
            </div>
          </footer>
        </article>
        <article
          className="box-border flex min-h-[21.75rem] w-[20rem] shrink-0 flex-col items-start justify-between self-stretch overflow-hidden rounded-lg bg-primary-2 px-[1rem] py-[1.5rem]"
          role="listitem"
        >
          <div className="flex h-[11.75rem] flex-col items-start justify-start gap-[0.75rem] self-stretch">
            <h3 className="relative inline-block h-[2.5rem] self-stretch font-medium leading-[1.25rem]">
              &quot;The best people I have ever worked with&quot;
            </h3>
            <div className="relative flex-1 self-stretch font-paragraph-regular-semibold text-[0.88rem] leading-[1.25rem]">
              <p className="[margin-block-end:16px] [margin-block-start:0]">
                &quot;Incredible group of people and talented professionals.
                Focused on the development of flexible and new stuffs.
              </p>
              <p className="m-0">
                Very inspiring working experience with their representatives,
                responsible and active in communication&quot;
              </p>
            </div>
          </div>
          <footer className="font-satoshi flex h-[2.5rem] flex-col items-start justify-start gap-[0.5rem] self-stretch text-[1.25rem]">
            <div className="relative flex-1 self-stretch font-medium leading-[1rem]">
              Aniruddho Sen
            </div>
            <div className="relative flex-1 self-stretch text-[0.88rem] leading-[1rem]">
              Vice-president, SenIT
            </div>
          </footer>
        </article>
      </div>
    </section>
  );
};

export default Testimonials;
